import {GEOLOCATION_REJECT_MESSAGE, TIMEOUT_SEC} from "./config";
...
export const getJSON = async function(url){
    try{
        ...
        if(!response.ok) throw new Error(`Something went wrong: ${response.statusText || response.message} (${response.status || 408})`)
